pub fn FuseOps(Shared: type) type {
    return struct {
        ...
        pub fn getattr(path: []const u8) error{ENOENT}!fuse.Stat {
            ...
            if (std.mem.eql(u8, path, "/")) {
                ...
                ret.nlink = 1;
                ret.size = @intCast(Shared.clipboard.len);
            } else {
                return error.ENOENT;
            }
            return ret;
        ...
        pub fn read(
            ...
            if (offset >= Shared.clipboard.len) {
                // already all read
                return 0;
            }
            // how much can we copy in
            const s = if (offset + buf.len > Shared.clipboard.len)
                // all the rest
                Shared.clipboard.len - offset
            else
                // only what fits
                buf.len;

            @memcpy(buf[0..s], Shared.clipboard[offset .. offset + s]);
